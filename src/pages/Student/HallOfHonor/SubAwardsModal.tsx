...
import type { AwardCategory, SubAward } from '../../../types';
...
const SubAwardsModal: React.FC<SubAwardsModalProps> = ({
  ...
  const [selectedMode, setSelectedMode] = useState<'custom' | 'customWithDescription' | 'schoolYear' | ''>('');
  ...
  useEffect(() => {
    if (open && category) {
      ...
      // Detect current mode
      if (category.subAwards.some(s => s.type === 'custom')) setSelectedMode('custom');
      else if (category.subAwards.some(s => s.type === 'custom_with_description')) setSelectedMode('customWithDescription');
      else if (category.subAwards.some(s => ['year', 'semester', 'month', 'schoolYear'].includes(s.type))) setSelectedMode('schoolYear');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="min-w-[40vw] max-w-[70vw] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={() => { setSelectedMode('custom'); setSubAwards([]); }}
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="awardType"
                value="customWithDescription"
                checked={selectedMode === 'customWithDescription'}
                onChange={() => { setSelectedMode('customWithDescription'); setSubAwards([]); }}
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="awardType"
                value="schoolYear"
                checked={selectedMode === 'schoolYear'}
                onChange={() => { setSelectedMode('schoolYear'); setSubAwards([]); }}
              />
          ...
          {selectedMode === 'customWithDescription' && (
            <div className="space-y-4">
